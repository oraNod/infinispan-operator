...
[NOTE]
====
If you do not specify credentials when you create {brandname} clusters, the {ispn_operator} automatically generates them.
====
...
+
* `example-infinispan-mgmt-generated-secret` contains credentials for the administrative user.
* `example-infinispan-app-generated-secret` contains credentials for the application user.
+
. Retrieve the credentials from the secret, for example:
.. Get the username for the application user.
+
[source,options="nowrap",subs=attributes+]
----
$ oc get secret example-infinispan-app-generated-secret -n my_namespace -o jsonpath="{.data.username}" | base64 --decode
----
+
.. Get the password for the application user.
+
[source,options="nowrap",subs=attributes+]
----
